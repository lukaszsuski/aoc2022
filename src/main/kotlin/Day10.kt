...
class FrameBuffer(val width: Int, height: Int) {
    ...
    fun drawFrame(): String {
        return line.windowed(width, width)
            .map { it.joinToString("") }
            .joinToString("\n")
    }
}
...
class Day10(input: List<String>) : Day(input) {
    ...
    override fun part1(): Any? = probe.signalStrength //14540

    override fun part2(): Any? = fb.drawFrame() //EHZFZHCZ
